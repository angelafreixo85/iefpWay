            ...
            <div class="row">
                <div class="col-sm-6 col-sm-offset-3 form-box">
                    ...
                    if (isset($_POST["form-username"]) && isset($_POST["form-password"])) {
                        ...
                        $sql = $conn->prepare("SELECT * FROM User WHERE (name = ? OR email = ?) AND password = ?");
                        $sql->bind_param("sss", $_POST["form-username"], $_POST["form-username"], $_POST["form-password"]);
                        $sql->execute();
                        $result = $sql->get_result();

                        if (mysqli_error($conn)) { //PB  ira aparecer um alerta dos dados do utilizador e password estiveram errados
                            ?>
                            <div class="alert alert-danger" role="alert">
                                User or password invalid
                            </div>
                        <?php
                        } else if (mysqli_num_rows($result) > 0) { //PB  verifica se a base de dados está vazia,
                        // se estiver valido aparece mensagem  login com sucesso.
